...
module RUbertooth
  ...
  class Ubertooth
    ...
    def initialize
      ...
      @input_endpoint = @device.endpoints.find do |ep|
        ep.bEndpointAddress & LIBUSB::ENDPOINT_IN != 0
      end
    ...
    def do_something(data)
      ...
      fail "Failed to send data: #{sent}" unless sent == data.size
    ...
    def modulation=(modulation)
      ...
      fail "Command failed: #{r}" unless r == 0
    end
    ...
    def access_address
      ...
      fail 'Command failed.' unless data && data.size == 4

      data[0] | data[1] << 8 | data[2] << 16 | data[3] << 24
    end

    def access_address=(address)
      data = [address].pack 'I'
      sent = @iface.control_transfer(
        bmRequestType: CTRL_OUT,
    ...
    def btle_slave(mac_address)
      ...
      fail "Failed to send data: #{sent}" unless sent == mac_address.size
    end
    ...
    def poll
      ...
      pkt = data.size == UsbPktRx::SIZE ? UsbPktRx.from_s(data) : nil
